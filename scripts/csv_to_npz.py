...
input_group.add_argument("--input_file", type=str, help="The path to the input motion csv file.")
input_group.add_argument("--input_dir", type=str, help="Directory containing motion files to batch convert.")
...
def run_simulator(sim: sim_utils.SimulationContext,
    ...
    frame_dt = motion.output_dt if motion.output_dt > 0 else sim.get_physics_dt()
...
def main():
    ...
    if args_cli.input_file:
        ...
        motion_files = sorted([p for p in Path(args_cli.input_dir).rglob("*.csv") if p.is_file()])

    if not motion_files:
        print("[WARN]: No motion files found to process.")
        return
